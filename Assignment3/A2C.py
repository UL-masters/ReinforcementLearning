...
class ValueNetwork(nn.Module):
    ...
    # forward pass to get scalar state value estimate V(s)
    def forward(self, x: torch.Tensor) -> torch.Tensor:
...
def a2c_update(
    ...
    # stack states and compute V(s) for each visited state
    states_tensor = torch.stack(states)
    values = value_net(states_tensor)

    # advantage: A(s,a) = Q(s,a) - V(s)
    # detach values so the actor gradient does not flow into the critic
    ...
    # entropy bonus: recompute distributions to get H[pi(.|s)] for each state
    entropy = torch.stack([
        policy.get_distribution(s).entropy() for s in states
    ]).mean()

    # negative because we want gradient ASCENT on J(theta)
    # subtract entropy bonus to encourage exploration
    actor_loss = -(log_probs_tensor * advantages).sum() - ENTROPY_COEF * entropy

    actor_optimizer.zero_grad()
    actor_loss.backward()
    actor_optimizer.step()

    # MSE between predicted V(s) and MC return
    # re-forward after actor step so gradients are clean
    values_new = value_net(states_tensor)
    critic_loss = nn.functional.mse_loss(values_new, returns)
...
# train actor and critic using A2C and return list of episode returns
def train() -> list:
    ...
    for episode in range(1, N_EPISODES + 1):
        ...
        # perform A2C update for both actor and critic using collected episode data
...
def plot_returns(returns: list, window: int = 50):
    ...
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig("a2c_learning_curve.png", dpi=150)
    plt.show()
    print("Plot saved to a2c_learning_curve.png")
